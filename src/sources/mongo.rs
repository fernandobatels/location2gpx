...
fn parse_doc(doc: &Document) -> Result<DevicePosition, String> {

    let device_id = doc.get_str("device")
        .map_err(|e| format!("Failed on access the `device`: {}", e.to_string()))?;

    let coordinates = doc.get_array("coordinates")
...
pub mod tests {
    ...
    fn mongo_track() -> Result<(), String> {
        ...
        let source = MongoDbSource::new(collection);
